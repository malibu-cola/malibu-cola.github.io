...
- 系統的な赤方偏移は$z=0.0232, V_r = 6888 {\rm km/s}$, ランダム運動は$\sqrt{\langle V^2\rangle} = 860{\rm km/s}$
- 視野角$6^\circ$程度
...
&=\frac{5.06{\rm Mpc} \times 3\times (860{\rm km/s})^2}{6.67 \times 10^{-11} {\rm N m^2 kg^{-2}}}\\
&=\frac{(5.06 \times 3.086 \times 10^{22} {\rm m}) \times 3\times(7.386 \times 10^{11}\rm m^2 s^{-2})}{6.67 \times 10^{-11}{\rm m^3 s^{-2} kg^{-1}}}\\
&= 5.193 \times 10^{45}{\rm kg} = 2.6 \times 10^{15} M_\odot
\end{align}
$$
...
$$
\frac{(M/M_\odot)}{(L/L_\odot) }=\frac{2.6 \times 10^{15}}{1.05 \times 10^{13}}  = 248
$$

**ダークマターの量の推定**

銀河の平均的な質量光度比は$M/L \sim 10$程度である。かみのけざ銀河団の総光度から見積もられる全質量は

$$
\begin{align}
M_{\text{optical}}/M_\odot &= \frac{M/M_\odot}{L/L_\odot} \times (L/L_\odot) \sim 10 \times 1.05 \times 10^{13} = 1.05 \times 10^{14}\\
\end{align}
$$

となる。となると、見えない物質(すなわちダークマター)による質量は

$$
\begin{align}
M_{\rm DM} &= M_{\rm total} - M_{\rm optical}\\
&=(2.6 \times 10^{15}  - 1.05 \times 10^{14})M_\odot\\
&=2.5 \times 10^{15} M_\odot
\end{align}
$$

となり、$2.5/2.6 \times 100=96\%$がダークマターだと推定される。
